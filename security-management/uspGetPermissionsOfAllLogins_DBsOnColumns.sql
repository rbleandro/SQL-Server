...
--SELECT @var,@ParmDefinition,@var1,@db_name,@sql_text,@RoleName,@UserName,@DBN,@sqlText,@d1,@d2,@d3,@ss,@ColN,@tSQLText;
...
BEGIN TRY
    ...
    BEGIN 
        ...
        BEGIN 
            ...
            SET @ss =
            'UPDATE ##db_name SET ' + QUOTENAME(@d2) + ' = ''' + @d3 + ''' WHERE LoginUserName = ''' + @d1 + '''' 
            EXEC (@ss);
        ...
        BEGIN 
            ...
            SET @var =
            '  UPDATE ##db_name SET ' + QUOTENAME(@d2) + '=''' + @var1 + ' ' + @d3 + ''' WHERE LoginUserName = ''' + @d1 + '''  ';
