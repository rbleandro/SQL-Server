...
BEGIN
	...
	OR message LIKE 'Replication-Replication%'
	OR (message LIKE 'TCP Provider:%' AND DATEPART(WEEKDAY,GETDATE()) IN (1,7))
	OR (message LIKE 'Communication link failure%' AND DATEPART(WEEKDAY,GETDATE()) IN (1,7))
	OR (message LIKE 'The process could not execute%' AND DATEPART(WEEKDAY,GETDATE()) IN (1,7))
	OR (message LIKE 'SQL Server Network Interfaces%' AND DATEPART(WEEKDAY,GETDATE()) IN (1,7))
	OR (message LIKE '%No user action is required%')
