...
WHERE TYP.Name IN ('varchar','nvarchar','text','sysname','char') -- Filtra o tipo de dado que você procura para evitar a procura de uma string em um inteiro
ORDER BY TAB.Name
...
BEGIN
	--PRINT @TabNome;
	SET @sql=N'select @retvalOUT=count(*) from (Select top 1 * From ['+ @schema +'].[' + @TabNome + '] Where [' + @ColNome + '] Like ''%fsc%'') as a'
	PRINT @sql
